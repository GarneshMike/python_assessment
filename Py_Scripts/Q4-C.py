...
# Define Column
columns = ["issue", "submitted_via"]
df_csv = pd.read_csv(csv_file, usecols=columns)

df_issue_count = df_csv.groupby(['submitted_via'])['issue'].count()
...
plt.text(total_x / 2, max_val - 3000, summary_value, size=15, rotation=0,
...
plt.ylabel('Count')
plt.title('Submission VS Issue(Count) ')
plt.show()
